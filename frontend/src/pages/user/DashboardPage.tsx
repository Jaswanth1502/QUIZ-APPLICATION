  ...
  return <section className="container-page py-10">
    ...
    {data && <>
      <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-5">
        <StatCard label="Completed quizzes" value={data.statistics.completedQuizzes ?? 0} />
        <StatCard label="Best score" value={`${data.statistics.bestScore ?? 0}%`} />
        <StatCard label="Average score" value={`${data.statistics.averageScore ?? 0}%`} />
        <StatCard label="Recent activity" value={data.recent.length} />
      </div>
      <div className="card p-7 mt-8">
        ...
        {data.recent?.length ? <div className="table-wrap">
          <table className="table">
            <thead><tr><th>Quiz</th><th>Score</th><th>Result</th><th></th></tr></thead>
            <tbody>{data.recent.map(a => <tr key={a.id}>
